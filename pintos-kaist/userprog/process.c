...
__do_fork (void *aux) {
	...
	// lock_acquire(&file_lock);
	// current->run_file = file_duplicate(parent->run_file);
	// lock_release(&file_lock);
	process_init ();
...
process_exit (void) {
	...
	// 	c->exit_status = -1;
	// 	sema_up(&c->sema);
	// } // 자신(부모)가 죽을 때 self_info 수정
		
	// for(e=list_begin(&cur->child_list); e!=list_end(&cur->child_list); ){
	// 		struct child *ch = list_entry(e, struct child, elem);
	// 		struct list_elem *next = list_next(e);
	// 		if(ch->is_exit && !ch->is_waited){
	// 			list_remove(e);
	// 			free(ch);
	// 		}
	// 		else{
	...
	// 	} // 자식 구조체 들어가서 자식이 먼저 죽은 상황이면 구조체 free 해주고 
	// 	// 지(부모)가 먼저 죽는 상황이면 구조체 내부의 parent_dead 바꿔줌
