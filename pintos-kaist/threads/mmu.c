...
/* 가상 주소 VADDR에 대한 페이지 맵 레벨 4(pml4)의  
   페이지 테이블 항목의 주소를 반환합니다.  
   PML4E가 VADDR에 대한 페이지 테이블을 가지고 있지 않다면,  
   동작은 CREATE 값에 따라 달라집니다.  
   CREATE가 true이면 새로운 페이지 테이블이 생성되며,  
   그 안의 포인터가 반환됩니다.  
...
/* 새로운 페이지 맵 레벨 4(pml4)를 생성합니다.  
   이 pml4는 커널 가상 주소에 대한 매핑은 가지고 있지만,  
   사용자 가상 주소에 대한 매핑은 없습니다.  
   새로운 페이지 디렉터리를 반환하며,  
   만약 메모리 할당에 실패하면 null 포인터를 반환합니다.*/
uint64_t *
pml4_create (void) {
...
/* 커널의 항목을 포함하여 사용 가능한 각 PTE 항목에 FUNC를 적용합니다.*/
bool
...
/* pml4에서 사용자 가상 주소 UADDR에 대응하는  
   물리 주소를 조회합니다.  
   그 물리 주소에 대응하는 커널 가상 주소를 반환하며,  
   UADDR이 매핑되어 있지 않으면 null 포인터를 반환합니다.
 */
void *
...
/* PML4 (Page Map Level 4)에 사용자 가상 페이지 UPAGE를 
   커널 가상 주소 KPAGE가 식별하는 물리 프레임과 매핑합니다.
   UPAGE는 이미 매핑되어 있지 않아야 하며,
   KPAGE는 palloc_get_page()를 통해 유저 풀에서 할당된 페이지일 가능성이 높습니다.

   WRITABLE이 true이면, 새 페이지는 **읽기/쓰기 가능(read/write)**하고,
   아니라면 **읽기 전용(read-only)**으로 설정됩니다.
...
/* 페이지 디렉터리 PD에서 사용자 가상 페이지 UPAGE를  
   ...
   이후 해당 페이지에 접근하면 페이지 폴트가 발생하게 됩니다.  
   페이지 테이블 항목의 다른 비트들은 그대로 유지됩니다.  
   UPAGE는 반드시 매핑되어 있을 필요는 없습니다.
 */
void
...
/* PML4에서 가상 페이지 VPAGE에 대한 PTE가  
   더럽혀졌는지(dirty) 여부를 반환합니다.  
   즉, PTE가 설정된 이후 해당 페이지가 수정되었는지를 나타냅니다.  
   PML4에 VPAGE에 대한 PTE가 존재하지 않으면 false를 반환합니다. */
bool
...
/* PML4에서 가상 페이지 VPAGE에 대한 PTE의 dirty 비트를 DIRTY 값으로 설정합니다. */
void
...
/* PML4에서 가상 페이지 VPAGE에 대한 PTE가  
   최근에 접근되었는지 여부를 반환합니다.  
   즉, PTE가 설정된 시점부터 마지막으로 해당 비트가  
   초기화된 시점까지의 접근 여부를 의미합니다.  
   PML4에 VPAGE에 대한 PTE가 존재하지 않으면 false를 반환합니다.
 */
bool
...
/* 페이지 디렉터리 PD에서 가상 페이지 VPAGE에 대한 PTE의  
   접근(accessed) 비트를 ACCESSED 값으로 설정합니다.*/
void
